...
static bool possiblyAffectedBy(const Instruction *QueryInst,
  ...
  if (isa<LoadInst>(DefInst) && isa<LoadInst>(QueryInst)) {
    const auto *DefLI = cast<LoadInst>(DefInst);
    const auto *QueryLI = cast<LoadInst>(QueryInst);
...
bool CachingMemorySSAWalker::instructionClobbersQuery(
  ...
  if (!Q.IsCall) {
    return possiblyAffectedBy(Q.Inst, DefMemoryInst) &&
           (AA->getModRefInfo(DefMemoryInst, Loc) & MRI_Mod);
  }
